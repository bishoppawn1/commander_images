...
The contact sheet is for preview only. Print the eight individual PNG files at their embedded 1200 DPI size, or use the [Letter-size grid](print_sheets/README.md). That folder contains an editable eight-layer GIMP XCF and a matching actual-size PDF. The separate [six-sheet GIMP arrangement tool and instructions](../../GIMP_GRIDS.md) live at the project root and target the user's verified cross-set layouts.

The current revision follows the compact hierarchy demonstrated by the supplied physical-label references: authentic mana symbols remain at the top, while the large one-line deck title, regular-weight strategy line, and smaller one-line product title are grouped closely at the bottom. Every text line uses neutral true white (RGB 255, 255, 255), with no ivory tint and no black outline; a soft shadow and the naturally dark footer area provide contrast. This leaves a large uninterrupted region of commander artwork visible through the center. Breed Lethality uses a brighter silver-blue Atraxa scene so it remains legible when printed. All text and graphic elements are rendered directly at the final 3300 × 2800 resolution.
